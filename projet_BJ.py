...
def regle_jeux():
    ...
    label_regle = tk.Label(
        regle,
        text=(
            "Le but du Blackjack est avoir une main la plus proche de 21 sans la depasser "
            "et de battre le croupier. Les cartes 2 a 10 valent leur valeur nominale. "
            "Les figures (Valet Dame Roi) valent 10. As vaut 1 ou 11 selon ce qui te convient. "
            "Au debut le joueur et le croupier recoivent chacun 2 cartes. Le croupier a une carte cachee. "
            "Le joueur peut soit tirer une carte de plus (Hit) soit rester avec ses cartes (Stand). "
            "Ensuite le croupier revele sa carte cachee et tire des cartes jusqu a atteindre au moins 17. "
            "Si le joueur depasse 21 il perd directement. Si le joueur est plus proche de 21 que le croupier "
            "il gagne. En cas d egalite la mise est remboursee. Si le joueur a un Blackjack (As + carte a 10) "
            "des le depart il gagne 1.5 fois sa mise."
        ),
        font=("Rockwell", 24),
        fg="white",
        bg="#1a3a1a",
...
def jeux():
    ...
    btn_solo = tk.Button(feujeux, text="SOLO", width=15, height=2, font=("Rockwell", 19), command=lambda: so.solo(fenetre), bg="#52be8c")
    btn_solo.place(relx=0.5, rely=0.4, anchor="center")
 
    btn_multi = tk.Button(feujeux, text="MULTIJOUEUR", width=15, height=2, font=("Rockwell", 19), command=None, bg="#52be8c")
    ...
    btn_rmenu.place(relx=0.9, rely=0.9, anchor="center")
