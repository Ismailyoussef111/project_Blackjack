    ...
    def valider_mise():
        ...
        try:
            ...
            if 0 < m <= argent_joueur: # Si on as assez d'argent
                ...
                btn_tirage.config(state="normal") # On ALLUME le bouton reprendre
                btn_reste.config(state="normal")   # On ALLUME le bouton garder
...
# dans fin croupier() et rester()
    ...
    sj, sc = val_main(joueur), val_main(croupier) # Les scores
    
    if sj <= 21 and (sc > 21 or sj > sc):
        argent_joueur += mise_actuelle.get() # On AJOUTE la mise si on gagnes
    elif sj < sc or sj > 21:
